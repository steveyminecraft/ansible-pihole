...
| `upgrade` | [`molecule/upgrade/`](molecule/upgrade/) | Previous Galaxy release on both Vagrant nodes, then `update-pihole` with Traefik enabled. `UPGRADE_FROM_VERSION=1.9.4 molecule test -s upgrade` |
...
Replace `VERSION` with the published release required, for example `1.9.5`. <!-- x-release-please-version -->
